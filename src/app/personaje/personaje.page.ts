...
export class PersonajePage implements OnInit {
  ...
  public get personajeVivo() {
    return this.personajes.filter((proc) => proc.status === 'Alive');
  }

  public get personajeMuerto() {
    return this.personajes.filter((proc) => proc.status === 'Dead');
  }

  public asesinarPersonaje(idPersonaje: number) {
    for (let per of this.personajes) {
      if (per.id === idPersonaje) per.status = 'Dead';
      break;
    }
  }

  public revivirPersonaje(idPersonaje: number) {
    for (let per of this.personajes) {
      if (per.id === idPersonaje) per.status = 'Alive';
      break;
    }
  }
